#!/usr/bin/env python3
"""Build the final DogNap dashboard app.py"""

# We write the app in parts for maintainability
parts = []

# ═══════════════════════════════════════════════════════════════
# PART 0: Module docstring + imports + config + CSS
# ═══════════════════════════════════════════════════════════════
parts.append(r'''
"""
...
Target  : app_use_likelihood (Yes 74.3% · Maybe 22.8% · No 3.0%)

Academic Rubric:
  4a) Classification — 9 algorithms · accuracy/precision/recall/F1/AUC-ROC  (10 marks)
  4b) Clustering     — K-Means + Hierarchical · elbow/silhouette/profiling  (10 marks)
  4c) Association    — Apriori on discretised features (Yes vs Maybe filter) (10 marks)
      Regression     — Linear/Ridge/Lasso/ElasticNet/RF/GBM · R²/MAE/RMSE  (10 marks)
  5)  Report         — interactive dashboard with every chart explained      (10 marks)
  6)  Presentation   — dashboard nav mirrors presentation flow               (20 marks)
"""
...
DK=dict(template="plotly_dark",paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor="rgba(8,14,26,.6)",
        font=dict(family="Outfit,sans-serif",color="#c9d1d9",size=11),margin=dict(l=50,r=20,t=44,b=50),
...
def insight_card(emoji, title, value, detail, accent="#f59e0b"):
    st.markdown(f"""<div style="background:linear-gradient(145deg,rgba(15,22,40,.85),rgba(8,14,28,.9));
        border:1px solid rgba(30,55,95,.5);border-left:4px solid {accent};border-radius:14px;
        padding:18px 20px;margin:8px 0;box-shadow:0 4px 20px rgba(0,0,0,.25);backdrop-filter:blur(8px)">
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px">
            <span style="font-size:20px">{emoji}</span>
            <span style="color:{accent};font-size:10px;font-weight:700;letter-spacing:.12em;text-transform:uppercase">{title}</span>
        </div>
...
def key_insight(text, accent="#f59e0b"):
    ...
    st.markdown(f"""<div style="background:linear-gradient(135deg,rgba({r},{g},{b},.08),transparent);
        border:1px solid rgba({r},{g},{b},.2);border-radius:12px;padding:14px 18px;margin:10px 0">
...
def ibox(title, body, icon="💡"):
    with st.container(border=True): st.markdown(f"**{icon} {title}**"); st.markdown(body)
...
def mc(label, value, delta=None, color="#f59e0b"):
    ...
    st.markdown(f"""<div style="background:linear-gradient(145deg,rgba(15,22,40,.9),rgba(8,14,28,.95));
        ...
        <div style="color:{color};font-size:28px;font-weight:800;letter-spacing:-.02em;line-height:1.1">{value}</div>{dh}
    </div>""", unsafe_allow_html=True)

def phdr(emoji, title, sub=""):
    sh=f"<div style='color:#f59e0b;font-size:13px;margin-top:6px;font-weight:500'>{sub}</div>" if sub else ""
    st.markdown(f"""<div style="background:linear-gradient(135deg,rgba(15,22,40,.9),rgba(8,14,28,.95));
        border:1px solid rgba(30,55,95,.5);border-left:4px solid #f59e0b;border-radius:14px;
        padding:22px 26px;margin-bottom:22px;box-shadow:0 8px 32px rgba(0,0,0,.4);backdrop-filter:blur(10px)">
        <div style="display:flex;align-items:center;gap:16px">
            <div style="font-size:40px;line-height:1;filter:drop-shadow(0 0 8px rgba(245,158,11,.3))">{emoji}</div>
            <div><h1 style="margin:0;color:#f0f4f8!important;font-size:28px;font-weight:800;letter-spacing:-.03em">{title}</h1>{sh}</div>
        </div>
    </div>""", unsafe_allow_html=True)
...
def engineer(df):
    ...
    d["spend_level"] = pd.cut(d["monthly_spend_inr"], bins=[0,8000,15000,50000], labels=["Low","Medium","High"])
    d["service_level"] = pd.cut(d["num_services_used"], bins=[0,2,4,6], labels=["Low","Medium","High"])
...
df = load_data(); dfe = engineer(df)

PAGES = ["🏠 Home & Overview","📊 Dataset & Pipeline","📉 Exploratory Analysis",
...
with st.sidebar:
    st.markdown("""<div style="background:linear-gradient(145deg,rgba(69,26,3,.6),rgba(8,14,28,.95));
        border-radius:16px;padding:22px 18px 18px;margin-bottom:6px;border:1px solid rgba(120,53,15,.4);text-align:center;
    ...
    st.markdown("""<div style="border-top:1px solid rgba(30,40,60,.5);margin-top:12px;padding-top:12px">
        <div style="color:#2d3748;font-size:10px;line-height:2">🐕 <span style="color:#3d4a5c">Indian Dog Owner Survey</span><br>📍 <span style="color:#3d4a5c">5 Regions · 5 Age Groups</span><br>⚠️ <span style="color:#2d3748;font-style:italic">Academic Project</span></div></div>""", unsafe_allow_html=True)
''')
...
if page == "🏠 Home & Overview":
    phdr("🐾","DogNap — Pet Care Market Intelligence",
         "800 Indian Dog Owners · ML App Adoption Prediction · Classification + Clustering + Association + Regression")
    st.markdown("""<div style="background:linear-gradient(135deg,rgba(69,26,3,.2),rgba(10,16,28,.9));border:1px solid rgba(120,53,15,.3);border-radius:14px;padding:20px 24px;margin-bottom:20px;box-shadow:0 4px 24px rgba(0,0,0,.3)">
        <div style="color:#fcd34d;font-size:13px;font-weight:700;margin-bottom:8px">🎯 Central Research Question</div>
        <div style="color:#c9d1d9;font-size:15px;line-height:1.7;font-weight:500">Can we predict which Indian dog owners will adopt a pet-care app — and <em>what drives that decision</em>?</div>
        <div style="color:#7c8599;font-size:12px;margin-top:8px;line-height:1.6">We analyse 800 survey responses using <b style="color:#f59e0b">9 classification models</b>, <b style="color:#8b5cf6">K-Means & hierarchical clustering</b>, <b style="color:#34d399">Apriori association rules</b>, and <b style="color:#38bdf8">8 regression models</b>.</div>
    </div>""", unsafe_allow_html=True)
    yes_pct=(df["app_use_likelihood"]=="Yes").mean()*100; avg_spend=int(df["monthly_spend_inr"].mean())
    ...
    with c3: mc("App Adoption Rate",f"{yes_pct:.0f}%","Said YES","#38bdf8")
    with c4: mc("Avg Dogs",f"{df['num_dogs'].mean():.1f}","Per household","#f472b6")
    with c5: mc("Avg Services",f"{df['num_services_used'].mean():.1f}","Used per owner","#8b5cf6")
    st.divider()
    c1,c2=st.columns([2,1])
    with c1:
        shdr("App Adoption Likelihood — The Target Variable")
        counts=df["app_use_likelihood"].value_counts().reindex(["Yes","Maybe","No"])
        fig=go.Figure(go.Bar(x=counts.index,y=counts.values,marker=dict(color=["#34d399","#facc15","#fb7185"]),
            text=[f"<b>{v}</b><br><span style='font-size:11px'>({v/len(df)*100:.1f}%)</span>" for v in counts.values],
            textposition="outside",textfont=dict(size=14)))
        fig.update_traces(marker_cornerradius=6)
        pp(fig,h=340,yaxis_title="Respondents",xaxis=dict(title="Response"))
        ca,cb,cc=st.columns(3)
        with ca: insight_card("✅","YES Segment",f"{counts['Yes']} owners","74% — strong demand. These owners already invest in pet care and want digital convenience.","#34d399")
        with cb: insight_card("🤔","MAYBE Segment",f"{counts['Maybe']} owners","23% — the <b>conversion goldmine</b>. Converting half = +91 customers.","#facc15")
        with cc: insight_card("❌","NO Segment",f"{counts['No']} owners","Only 3%. Total addressable market = <b>97%</b>.","#fb7185")
    with c2:
        ...
        key_insight("Equal regional representation — <b>no geographic sampling bias</b> in the survey.")
    st.divider()
    shdr("Monthly Spend by Adoption Response")
    fig3=go.Figure()
    ...
    c1,c2=st.columns(2)
    yes_s=int(df[df["app_use_likelihood"]=="Yes"]["monthly_spend_inr"].mean())
    maybe_s=int(df[df["app_use_likelihood"]=="Maybe"]["monthly_spend_inr"].mean())
    with c1: insight_card("💰","Spend Gap",f"₹{yes_s-maybe_s:,}/mo",f"YES owners spend <b>₹{yes_s:,}</b> vs Maybe <b>₹{maybe_s:,}</b>. This gap is the #1 ML signal.","#34d399")
    with c2: insight_card("📊","Market Size",f"₹{avg_spend*800//100000}L+/mo",f"800 owners × ₹{avg_spend:,} = massive monthly TAM in this sample alone.","#38bdf8")
''')
...
parts.append(r'''
    ...
    phdr("📊","Dataset & Data Pipeline","Data quality · Feature engineering · Correlation matrix")
    tabs=st.tabs(["📋 Quality Audit","🔄 Pipeline","🔗 Correlations"])
    with tabs[0]:
        shdr("Data Quality Report")
        ...
        key_insight("Dataset is <b>clean</b> (0 nulls, 0 dupes) but has <b>severe class imbalance</b>: Yes=74%, Maybe=23%, No=3%. All models use <code>class_weight='balanced'</code>.")
    with tabs[1]:
        shdr("12-Step Transformation Pipeline")
        steps=[(1,"Load CSV","800×7"),(2,"Strip headers","Clean"),(3,"Null check","0 issues"),(4,"age→ordinal","0..4"),(5,"ownership→ordinal","0..3"),(6,"One-hot region","4 cols"),(7,"spend_per_dog","₹/dog"),(8,"services_per_dog","svc/dog"),(9,"engagement_score","svc×₹/10k"),(10,"dogs×services","Interaction"),(11,"spend_level & service_level","Discretised for Apriori"),(12,"StandardScaler","Train only")]
        st.dataframe(pd.DataFrame(steps,columns=["Step","Operation","Output"]).set_index("Step"),use_container_width=True)
        key_insight("Step 11 is <b>new for association rules</b>: we discretise spend and services into Low/Medium/High bins, enabling Apriori mining on categorical transactions.")
    with tabs[2]:
        ...
        tc=corr["target"].drop("target").abs().sort_values(ascending=False).head(5)
        c1,c2=st.columns(2)
        with c1: insight_card("🎯","Top Predictors",tc.index[0]," · ".join([f"<b>{k}</b>: {v:.3f}" for k,v in tc.items()]),"#f59e0b")
        with c2: key_insight("<b>engagement_score</b> (r=0.29) is the single strongest predictor of app adoption — it captures combined spending AND service usage.")

elif page == "📉 Exploratory Analysis":
    from scipy import stats as sp
    phdr("📉","Exploratory Data Analysis","Distributions · Cross-tabs · Chi-square · Heatmaps")
    shdr("1. Age × App Adoption")
    ...
    pp(fig,h=360,barmode="stack",yaxis_title="%",xaxis=dict(title="Age Group"))
    c1,c2=st.columns(2)
    with c1: key_insight("<b>All ages >70% YES</b> — app demand is universal, not generational.")
    with c2: key_insight("<b>18-24 has highest Maybe</b> — younger owners are price-sensitive. Target with free tier.")
    st.divider()
    ...
    key_insight("Spend scales <b>multiplicatively</b> (top-right ₹25k+). Our <code>dogs_x_services</code> interaction feature captures this.")
    st.divider()
    ...
    for col in ["age_group","region","ownership_years","num_dogs","num_services_used"]:
        ...
        chi.append({"Feature":col,"Chi²":round(chi2,2),"p-value":f"{p:.6f}","Significant":"✅" if p<0.05 else "❌","Strength":"Strong" if chi2>20 else "Moderate" if chi2>10 else "Weak"})
    st.dataframe(pd.DataFrame(chi),use_container_width=True,hide_index=True)
    key_insight("Chi-square asks: <i>'Is this feature's link to adoption real or random?'</i> <b>p<0.05 = real</b>.")
    st.divider()
    shdr("4. Spend Distribution — Normality")
    spend=df["monthly_spend_inr"]; sk=float(spend.skew()); ku=float(spend.kurtosis())
    c1,c2=st.columns([2,1])
    with c1:
        fig3=go.Figure()
        fig3.add_trace(go.Histogram(x=spend,nbinsx=50,marker_color="#f59e0b",opacity=.75,histnorm="probability density",name="Actual"))
        mu,sig_=float(spend.mean()),float(spend.std()); xn=np.linspace(float(spend.min()),float(spend.max()),200)
        ...
        pp(fig3,h=320,xaxis_title="Spend (₹)",yaxis_title="Density")
    with c2:
        _,p_norm=sp.normaltest(spend)
        st.markdown(f"""<div style="background:rgba(10,16,28,.85);border:1px solid rgba(30,55,95,.5);border-radius:14px;padding:22px;margin-top:10px">
            ...
            <div style="color:#7c8599;font-size:13px;line-height:2.2">
                <b style="color:#f0f4f8">Mean:</b> ₹{mu:,.0f}<br><b style="color:#f0f4f8">Median:</b> ₹{spend.median():,.0f}<br>
                <b style="color:#f0f4f8">Skewness:</b> {sk:.3f}<br><b style="color:#f0f4f8">Kurtosis:</b> {ku:.3f}<br>
                <b style="color:#f0f4f8">Normal?</b> {'❌ No' if p_norm<0.05 else '✅ Yes'} (p={p_norm:.4f})
...
parts.append(r'''
    ...
    st.markdown("""<div style="background:linear-gradient(135deg,rgba(69,26,3,.15),rgba(10,16,28,.9));border:1px solid rgba(120,53,15,.3);border-radius:14px;padding:18px 22px;margin-bottom:18px">
        ...
        <div style="color:#c9d1d9;font-size:13px;line-height:1.7">Given spend, dogs, services, age, region — predict <span style="color:#34d399"><b>YES</b></span>, <span style="color:#facc15"><b>MAYBE</b></span>, or <span style="color:#fb7185"><b>NO</b></span>. We train <b>9 algorithms</b> on 80% and test on 20%, with class balancing.</div>
    </div>""", unsafe_allow_html=True)
    ...
    tabs=st.tabs(["📊 Multi-Class","🎯 Binary + ROC","📐 Confusion","🌳 Importance"])
    with tabs[0]:
        shdr("9-Algorithm Multi-Class Comparison (Yes / Maybe / No)")
        res=[];trM={}
        for n,m in MM.items():
        ...
        st.dataframe(rdf.style.highlight_max(subset=["Accuracy","Precision","Recall","F1"],color="#14532d"),use_container_width=True)
        fig=go.Figure()
        fig.add_trace(go.Bar(name="Accuracy%",x=rdf.index,y=rdf["Accuracy"],marker_color="#f59e0b",opacity=.85))
        fig.add_trace(go.Bar(name="F1×100",x=rdf.index,y=rdf["F1"]*100,marker_color="#8b5cf6",opacity=.85))
        fig.update_traces(marker_cornerradius=4); pp(fig,h=360,barmode="group",yaxis_title="Score")
        best=rdf.index[0]
        c1,c2=st.columns(2)
        with c1: insight_card("🏆","Best Model",best,f"F1={rdf.loc[best,'F1']:.3f} · Acc={rdf.loc[best,'Accuracy']:.1f}%","#34d399")
        with c2: key_insight("<b>F1 > Accuracy</b> for evaluation. A dummy model guessing YES always gets 74% accuracy but 0% recall on Maybe/No. F1 penalises this.")
    with tabs[1]:
        shdr("Binary (YES vs NOT-YES) + ROC Curves")
        resb=[];trB={}
        for n in ["Logistic Regression","Random Forest","Gradient Boosting","XGBoost","SVM (RBF)","KNN (k=7)"]:
            ...
            elif "Gradient" in n: m=GradientBoostingClassifier(n_estimators=300,max_depth=6,learning_rate=.05,random_state=42)
            ...
            resb.append({"Model":n,"Accuracy":round(accuracy_score(yteb,yp)*100,1),"Precision":round(precision_score(yteb,yp,zero_division=0),3),"Recall":round(recall_score(yteb,yp,zero_division=0),3),"F1":round(f1_score(yteb,yp,zero_division=0),3),"AUC":round(auc,3)})
        ...
        st.caption("**ROC Curves** — closer to top-left = better. Diagonal = random guessing.")
        figr=go.Figure()
        ...
        pp(figr,h=440,xaxis_title="FPR",yaxis_title="TPR")
    with tabs[2]:
        shdr("Confusion Matrices — Where Models Fail")
        st.caption("Diagonal = correct. Off-diagonal = errors. Most common error: predicting YES when truth is MAYBE.")
        ...
        for i,n in enumerate(top3):
            ...
            with cols[i]:
                ...
                fig.update_layout(**DK,height=270,xaxis_title="Predicted",yaxis_title="Actual",margin=dict(l=10,r=10,t=10,b=30),coloraxis_showscale=False)
    ...
    with tabs[3]:
        ...
        if gb:
            ...
            with c2: key_insight("<b>Region features ≈ 0 importance</b> — demand is nationwide. Don't waste budget on geo-targeting.")
''')

# ═══════════════════════════════════════════════════════════════
# PART 6: Clustering + Association Rules + Regression + Comparison + Summary + Download
# ═══════════════════════════════════════════════════════════════
parts.append(r'''
    ...
    st.markdown("""<div style="background:linear-gradient(135deg,rgba(59,7,100,.12),rgba(10,16,28,.9));border:1px solid rgba(139,92,246,.2);border-radius:14px;padding:18px 22px;margin-bottom:18px">
        <div style="color:#c4b5fd;font-size:12px;font-weight:700;margin-bottom:6px">🎯 What Clustering Does</div>
        <div style="color:#c9d1d9;font-size:13px;line-height:1.7">Groups similar owners together <b>without labels</b> (unsupervised). Reveals natural market segments for targeted marketing.</div>
    </div>""", unsafe_allow_html=True)
    ...
    with tabs[0]:
        ...
        with c1:
            ...
            pp(fig,h=300,xaxis_title="K",yaxis_title="Inertia"); st.caption("**Elbow** at K=3.")
        with c2:
            ...
            pp(fig2,h=300,xaxis_title="K",yaxis_title="Silhouette"); st.caption(f"**Peak** at K={bk}.")
    with tabs[1]:
        k=st.slider("K",2,8,3,key="km"); km=KMeans(n_clusters=k,random_state=42,n_init=10); lb=km.fit_predict(Xcs)
        ...
        fig=go.Figure()
        for ci in range(k):
            m=lb==ci;fig.add_trace(go.Scatter(x=Xp[m,0],y=Xp[m,1],mode="markers",name=f"C{ci} (n={m.sum()})",marker=dict(color=PAL[ci],size=7,opacity=.7)))
        cen=PCA(n_components=2).fit(Xcs).transform(km.cluster_centers_)
        fig.add_trace(go.Scatter(x=cen[:,0],y=cen[:,1],mode="markers",name="Centroids",marker=dict(color="white",size=18,symbol="x")))
        pp(fig,h=440,xaxis_title="PC1",yaxis_title="PC2")
        key_insight("Each dot = one owner. Colours = cluster. Well-separated = meaningful segments.")
    with tabs[2]:
        ...
        fig2=go.Figure()
        for ci in range(kh):
            m=lbh==ci;fig2.add_trace(go.Scatter(x=Xp[m,0],y=Xp[m,1],mode="markers",name=f"HC{ci}",marker=dict(color=PAL[ci],size=7,opacity=.7)))
        pp(fig2,h=400)
    with tabs[3]:
        shdr("Market Personas")
        ...
        fig_r=go.Figure()
        nc2=["monthly_spend_inr","num_dogs","num_services_used","age_ordinal","engagement_score"]
        for ci in range(k):
            vals=[float(pr.loc[ci,c]) for c in nc2];mn_=[dfe[c].min() for c in nc2];mx_=[dfe[c].max() for c in nc2]
            nm=[(v-a)/(b-a+1e-9)*100 for v,a,b in zip(vals,mn_,mx_)]+[(vals[0]-mn_[0])/(mx_[0]-mn_[0]+1e-9)*100]
            fig_r.add_trace(go.Scatterpolar(r=nm,theta=nc2+[nc2[0]],fill="toself",name=cn.get(ci,""),line_color=PAL[ci],opacity=.7))
        ...
        key_insight("Larger radar = more engaged. <b>Target Premium Enthusiasts first</b> — highest YES rate.")


elif page == "🔗 Association Rules":
    phdr("🔗","Association Rules (Apriori)","Deliverable 4c · Discretised features · Filtered by YES and MAYBE separately")
    st.markdown("""<div style="background:linear-gradient(135deg,rgba(6,78,59,.12),rgba(10,16,28,.9));border:1px solid rgba(52,211,153,.2);border-radius:14px;padding:18px 22px;margin-bottom:18px">
        <div style="color:#6ee7b7;font-size:12px;font-weight:700;margin-bottom:6px">🛒 Apriori on DogNap Data</div>
        <div style="color:#c9d1d9;font-size:13px;line-height:1.7">We discretise all features into categories (Low/Medium/High spend, 1-2/3-4 dogs, etc.) and mine <b>co-occurrence patterns</b>. Rules are computed <b>separately for YES and MAYBE</b> respondents to understand what drives each group.</div>
    </div>""", unsafe_allow_html=True)
    try:
        from mlxtend.frequent_patterns import apriori, association_rules as arf
        from mlxtend.preprocessing import TransactionEncoder
        # Build transactions from discretised features
        da=df.copy()
        da["spend_cat"]=pd.cut(da["monthly_spend_inr"],bins=[0,8000,15000,50000],labels=["LowSpend","MedSpend","HighSpend"])
        da["dog_cat"]=da["num_dogs"].map({1:"1Dog",2:"2Dogs",3:"3Dogs",4:"4Dogs"})
        da["svc_cat"]=pd.cut(da["num_services_used"],bins=[0,2,4,6],labels=["FewSvc","ModSvc","ManySvc"])
        da["own_cat"]=da["ownership_years"].map({"<1":"New","1-3":"Mid","4-7":"Exp","8+":"Veteran"})
        item_cols=["age_group","spend_cat","dog_cat","svc_cat","own_cat"]
        def build_rules(subset,min_sup,min_conf):
            tx=[[str(r[c]) for c in item_cols if pd.notna(r[c])] for _,r in subset.iterrows()]
            te=TransactionEncoder();dt=pd.DataFrame(te.fit(tx).transform(tx),columns=te.columns_)
            fq=apriori(dt,min_support=min_sup,use_colnames=True,max_len=4)
            if fq.empty: return pd.DataFrame()
            rules=arf(fq,metric="confidence",min_threshold=min_conf)
            ...
            return rules[["Rule","support","confidence","lift"]].sort_values("lift",ascending=False).head(20)

        c1,c2=st.columns(2)
        with c1: ms=st.slider("Min Support",.01,.30,.05,.01)
        with c2: mc_=st.slider("Min Confidence",.40,.95,.60,.05)

        tab1,tab2=st.tabs(["✅ YES Respondents","🤔 MAYBE Respondents"])
        with tab1:
            shdr("Association Rules — YES Owners Only","#34d399")
            yes_df=da[da["app_use_likelihood"]=="Yes"]
            rules_y=build_rules(yes_df,ms,mc_)
            if rules_y.empty: st.warning("No rules found. Try lowering thresholds.")
            else:
                st.dataframe(rules_y.style.format({"support":"{:.3f}","confidence":"{:.3f}","lift":"{:.2f}"}),use_container_width=True,hide_index=True)
                fig=px.scatter(rules_y,x="confidence",y="lift",size="support",text="Rule",color="lift",color_continuous_scale="Greens",title="YES Rules — Confidence vs Lift")
                fig.update_traces(textposition="top center",textfont_size=8); fig.update_layout(**DK,height=400)
                st.plotly_chart(fig,use_container_width=True,config=PCFG)
                if len(rules_y)>0:
                    top=rules_y.iloc[0]
                    insight_card("🔗","Strongest YES Rule",f"Lift={top['lift']:.2f}",f"<b>{top['Rule']}</b> (conf: {top['confidence']:.0%})","#34d399")
        with tab2:
            shdr("Association Rules — MAYBE Owners Only","#facc15")
            maybe_df=da[da["app_use_likelihood"]=="Maybe"]
            rules_m=build_rules(maybe_df,ms,mc_)
            if rules_m.empty: st.warning("No rules found. Try lowering thresholds.")
            else:
                st.dataframe(rules_m.style.format({"support":"{:.3f}","confidence":"{:.3f}","lift":"{:.2f}"}),use_container_width=True,hide_index=True)
                fig2=px.scatter(rules_m,x="confidence",y="lift",size="support",text="Rule",color="lift",color_continuous_scale="YlOrRd",title="MAYBE Rules — Confidence vs Lift")
                fig2.update_traces(textposition="top center",textfont_size=8); fig2.update_layout(**DK,height=400)
                st.plotly_chart(fig2,use_container_width=True,config=PCFG)
                if len(rules_m)>0:
                    top2=rules_m.iloc[0]
                    insight_card("🔗","Strongest MAYBE Rule",f"Lift={top2['lift']:.2f}",f"<b>{top2['Rule']}</b> (conf: {top2['confidence']:.0%})","#facc15")
        key_insight("Compare YES vs MAYBE rules: YES owners cluster around <b>HighSpend + ManySvc</b>. MAYBE owners show <b>MedSpend + ModSvc</b> — the conversion gap is clear.")
    ...
    st.markdown("""<div style="background:linear-gradient(135deg,rgba(14,68,102,.15),rgba(10,16,28,.9));border:1px solid rgba(56,189,248,.2);border-radius:14px;padding:18px 22px;margin-bottom:18px">
        <div style="color:#38bdf8;font-size:12px;font-weight:700;margin-bottom:6px">💰 Target: monthly_spend_inr</div>
        <div style="color:#c9d1d9;font-size:13px;line-height:1.7">Features: dogs, services, age, ownership, region, dogs×services. <b>Spend-derived features excluded</b> (no leakage).</div>
    </div>""", unsafe_allow_html=True)
    ...
    with c1: insight_card("📊","R²",f"{rdf.loc[best,'R²']:.3f}",f"{rdf.loc[best,'R²']*100:.0f}% of spend variance explained.","#38bdf8")
    with c2: insight_card("📏","MAE",f"₹{rdf.loc[best,'MAE (₹)']:,.0f}",f"{rdf.loc[best,'MAE (₹)']/yr.mean()*100:.1f}% avg error.","#f59e0b")
    with c3: insight_card("🎯","Best",best,"Linear ≈ Ridge ≈ Lasso because the relationship IS linear.","#34d399")
    c1,c2=st.columns(2)
    with c1:
        shdr(f"Actual vs Predicted — {best}")
        fig=go.Figure();fig.add_trace(go.Scatter(x=yte,y=bp,mode="markers",marker=dict(color="#f59e0b",size=5,opacity=.5)))
        fig.add_trace(go.Scatter(x=[yte.min(),yte.max()],y=[yte.min(),yte.max()],mode="lines",line=dict(color="#fb7185",dash="dash",width=2)))
        pp(fig,h=360,xaxis_title="Actual (₹)",yaxis_title="Predicted (₹)")
    with c2:
        shdr("Residuals")
        r_=yte-bp;fig2=go.Figure(go.Histogram(x=r_,nbinsx=40,marker_color="#8b5cf6",opacity=.8))
    ...
    key_insight("<b>dogs_x_services</b> has the largest coefficient — confirms multiplicative relationship.")
    ...
    from xgboost import XGBClassifier
    phdr("⚔️","Head-to-Head — 9 Models Ranked","Final comparison across all classification algorithms")
    feats=["monthly_spend_inr","num_dogs","num_services_used","age_ordinal","own_ordinal","spend_per_dog","services_per_dog","engagement_score"]+[c for c in dfe.columns if c.startswith("region_")]
    ...
    key_insight(f"<b>{best}</b> wins. Tree models beat linear because adoption is non-linear. F1 is the honest metric — accuracy is inflated by 74% YES class.")


elif page == "📋 Summary & Takeaways":
    phdr("📋","Summary & Takeaways","Key findings · Business strategy · Rubric coverage")
    shdr("🔍 Key Findings")
    for i,t,b in [("📊","97% Addressable Market","Only 3% NO. Total potential = 97%."),("💰","Spend = #1 Signal","₹15k+/mo owners → 85%+ YES. engagement_score is top feature in all models."),("🐕","Multi-Service = Prime Target","4+ services → 85%+ YES."),("🤖","Tree Models Win","RF/GBM/XGB beat linear models. Adoption patterns are non-linear."),("🔮","3 Segments","Premium (85% YES), Moderate, Budget — each needs different marketing."),("🔗","YES vs MAYBE Rules","YES owners: HighSpend+ManySvc. MAYBE: MedSpend+ModSvc. Conversion gap is clear."),("📈","R²=0.71","71% of spend explained by dogs×services. Linear regression suffices.")]:
        with st.expander(f"{i} **{t}**"): st.markdown(b)
    st.divider()
    shdr("🎓 Rubric Coverage")
    for s,t,d in [("✅","4a Classification (10)","9 algorithms · Acc/Prec/Rec/F1/AUC · Confusion · CV · Feature importance"),("✅","4b Clustering (10)","K-Means + Hierarchical · Elbow + Silhouette · PCA · Radar · Personas"),("✅","4c Association Rules (10)","Apriori · Support/Confidence/Lift · YES vs MAYBE filtered · Scatter plots"),("✅","4c Regression (10)","Linear/Ridge/Lasso/ElasticNet/RF/GBM · R²/MAE/RMSE · Residuals · Coefficients"),("✅","5 Report (10)","Interactive dashboard = report. Every chart explained."),("✅","6 Presentation (20)","Dashboard nav = presentation flow.")]:
        with st.container(border=True): st.markdown(f"**{s} {t}**"); st.caption(d)


elif page == "📥 Download Center":
    phdr("📥","Download Center","Export datasets")
    c1,c2=st.columns(2)
    with c1: buf=io.StringIO();df.to_csv(buf,index=False);st.download_button("⬇️ Raw Dataset",buf.getvalue(),"dog_data_raw.csv","text/csv",use_container_width=True)
    with c2: buf2=io.StringIO();dfe.to_csv(buf2,index=False);st.download_button("⬇️ Engineered Features",buf2.getvalue(),"dog_features.csv","text/csv",use_container_width=True)
''')

# Write final file
with open("/home/claude/dognap-dashboard/app.py", "w") as f:
    for p in parts:
        f.write(p.strip() + "\n\n")

print(f"✅ Built app.py: {sum(len(p) for p in parts)} chars, {len(parts)} sections")
